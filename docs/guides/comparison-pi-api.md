...
| **安定性** | 低（頻繁変更前提の開発テスト用） | 高（外部アーキテクチャでプラグイン内部に非依存） |
...
| ドキュメント | △ SKILL.md のみ | ✅ ADR + recipes + safety | 新が圧倒的 |
| テスト | ❌ | ✅ pytest | 新のみ |
| Krita なしで動く | ❌ | ✅ ComfyUI 単体で可 | 新が圧倒的 |

---

## まとめ

新 `krita-agent-bridge` は旧 pi_api の主要機能をすべて実装しました。
